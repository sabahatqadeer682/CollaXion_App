  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await ax().put(`/api/industry/posts/${post._id}`, {
        ...
        seats, deadline, location, mode, skills, poster,
      }, { headers });
...
export function MyPostsScreen() {
  ...
  if (loading) return <View style={s.center}><ActivityIndicator size="large" color="#0066CC" /></View>;
  ...
  return (
    <View style={{ flex:1, backgroundColor:"#F0F4F8" }}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          {(["All","Internship","Project","Workshop"] as const).map(f => (
            <TouchableOpacity key={f} style={[s.filterChip, filter===f && s.filterChipActive]} onPress={() => setFilter(f)}>
              <Text style={[s.filterChipTxt, filter===f && s.filterChipTxtActive]}>{f}</Text>
              <View style={[s.filterBadge, filter===f && { backgroundColor:"#0066CC" }]}>
                <Text style={[s.filterBadgeTxt, filter===f && { color:"#fff" }]}>
                  {f==="All" ? posts.length : posts.filter(p => p.type===f).length}
                </Text>
              </View>
            </TouchableOpacity>
      ...
      <Animated.ScrollView style={{ opacity:fadeAnim }} contentContainerStyle={{ padding:16, paddingBottom:50 }}
        refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => fetchPosts(true)} colors={["#0066CC"]} />}
        showsVerticalScrollIndicator={false}>
        ...
        {filtered.length===0 ? (
          <View style={s.empty}>
            ...
            <Text style={s.emptyTitle}>Koi post nahi</Text>
            <Text style={s.emptySub}>{filter==="All" ? "Abhi tak koi post nahi ki" : `Koi ${filter} post nahi`}</Text>
            <TouchableOpacity style={s.emptyBtn} onPress={() => nav.navigate("PostOpportunity")}>
              <Text style={s.emptyBtnTxt}>+ Post Karo</Text>
            </TouchableOpacity>
          </View>
          ...
          return (
            <View key={post._id} style={[s.card, !post.isActive && s.cardInactive]}>
              ...
              <View style={s.cardBody}>
                ...
                <Text style={s.cardDesc} numberOfLines={2}>{post.description}</Text>
                <View style={s.metaRow}>
                  {post.mode && <View style={s.metaChip}><Ionicons name="location-outline" size={11} color="#0066CC" /><Text style={s.metaChipTxt}>{post.mode}</Text></View>}
                  {post.seats && <View style={s.metaChip}><Ionicons name="people-outline" size={11} color="#0066CC" /><Text style={s.metaChipTxt}>{post.seats} seats</Text></View>}
                  {post.deadline && <View style={s.metaChip}><Ionicons name="calendar-outline" size={11} color="#0066CC" /><Text style={s.metaChipTxt}>Due {post.deadline}</Text></View>}
                </View>
                {post.skills?.length>0 && (
                  <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginTop:8 }}>
                ...
                <View style={s.statsRow}>
                  ...
                  <View style={[s.statusDot, { backgroundColor:post.isActive?"#22C55E":"#94A3B8" }]} />
                  <Text style={[s.statusTxt, { color:post.isActive?"#16A34A":"#94A3B8" }]}>{post.isActive?"Active":"Hidden"}</Text>
                </View>
              </View>
...
const s = StyleSheet.create({
  ...
  newBtnTxt:{ fontSize:13, fontWeight:"700", color:"#fff" },
  filterBar:{ backgroundColor:"#fff", paddingHorizontal:12, paddingVertical:10, borderBottomWidth:1, borderColor:"#F1F5F9" },
  filterChip:{ flexDirection:"row", alignItems:"center", gap:6, paddingHorizontal:14, paddingVertical:7, borderRadius:20, borderWidth:1.5, borderColor:"#E2E8F0", marginRight:8 },
  filterChipActive:{ backgroundColor:"#0A1628", borderColor:"#0A1628" },
  filterChipTxt:{ fontSize:13, fontWeight:"600", color:"#64748B" },
  filterChipTxtActive:{ color:"#fff" },
  filterBadge:{ backgroundColor:"#F1F5F9", borderRadius:10, paddingHorizontal:6, paddingVertical:1 },
  filterBadgeTxt:{ fontSize:10, fontWeight:"900", color:"#64748B" },
  empty:{ alignItems:"center", paddingTop:80 },
  emptyIcon:{ width:72, height:72, borderRadius:36, justifyContent:"center", alignItems:"center", marginBottom:16 },
  emptyTitle:{ fontSize:18, fontWeight:"700", color:"#334155" },
  emptySub:{ fontSize:13, color:"#94A3B8", marginTop:6 },
