...
export const ToastProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const colors: Record<string, [string, string]> = { success: ["#d1fae5", "#059669"], error: ["#fee2e2", "#dc2626"], info: ["#dbeafe", "#2563eb"] };
  const [bg, fg] = colors[t?.type || "info"] || colors.info;
  return (
    <ToastCtx.Provider value={show}>
      {children}
      {t && (
        <Animated.View style={[sharedStyles.toast, { backgroundColor: bg, borderColor: fg + "66",
...
// ─── HEADER ──────────────────────────────────────────────────────
export const Header = ({ title, back = false, right }: { title: string; back?: boolean; right?: React.ReactNode }) => {
  const { useNavigation } = require("@react-navigation/native");
  const nav = useNavigation();
  return (
    <View style={sharedStyles.hdr}>
      <TouchableOpacity onPress={() => back ? nav.goBack() : nav.openDrawer()} style={sharedStyles.hdrBtn}>
...
export const FieldInput = ({
  ...
  placeholder?: string; multiline?: boolean; editable?: boolean; kbType?: any;
}) => (
  <View style={{ marginBottom: 12 }}>
    {label && <Text style={sharedStyles.fieldLbl}>{label}</Text>}
    <TextInput
      style={[sharedStyles.fieldInput, multiline && { height: 90, textAlignVertical: "top" }, !editable && { backgroundColor: "#F3F4F6", color: C.textLight }]}
