...
const AIRecommendationsScreen = () => {
    ...
    const fetchRecommendations = async () => {
        ...
        try {
            ...
            if (response.data.success) {
                setRecommendations(response.data.recommendations || []);
                setStudentSkills(response.data.studentSkills || []);
                if (response.data.message) setInfoMessage(response.data.message);
            }
        } catch (err: any) {
            if (err.response?.status === 404) {
                Alert.alert("Error", "Student profile not found");
    ...
    // Step 1: open cover letter input
    const handleApplyPress = (rec: any) => {
        const internship = rec.internshipId;
        if (!internship) return;

        if (appliedIds.includes(internship._id)) {
            Alert.alert("Already Applied", "You have already applied for this internship.");
            return;
        }

        setSelectedItem({ ...internship, matchScore: rec.matchScore });
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: "#F7F9FC" }}>
            ...
            >
                ...
                <View style={styles.header}>
                    ...
                    <Text style={styles.headerTitle}>AI-Powered Recommendations</Text>
                    <Text style={styles.headerSubtitle}>Based on your skills and profile</Text>
                </View>
                ...
                {recommendations.length === 0 && studentSkills.length === 0 ? (
                    ...
                    <>
                        ...
                        {recommendations.map((rec: any, idx: number) => {
                            const isApplied = appliedIds.includes(rec.internshipId?._id);
                            return (
                                <View key={idx} style={styles.recommendationCard}>
                                    {rec.internshipId?.image && (
                                        <Image
                                    ...
                                    {rec.matchingSkills?.length > 0 && (
                                        <View style={styles.skillSection}>
                                            ...
                                            <View style={styles.skillRow}>
                                                {rec.matchingSkills.map((skill: string, i: number) => (
                                                    <View key={i} style={[styles.skillBadge, { backgroundColor: "#D5F5E3" }]}>
                                                        <MaterialIcons name="check-circle" size={14} color="#27AE60" />
                                                        <Text style={styles.skillText}>{skill}</Text>
                                                    </View>
                                                ))}
                                    ...
                                    {rec.missingSkills?.length > 0 && (
                                        <View style={styles.skillSection}>
                                            ...
                                            <View style={styles.skillRow}>
                                                {rec.missingSkills.map((skill: string, i: number) => (
                                                    <View key={i} style={[styles.skillBadge, { backgroundColor: "#FDEDEC" }]}>
                                                        <MaterialIcons name="cancel" size={14} color="#E74C3C" />
                                                        <Text style={styles.skillText}>{skill}</Text>
                                                    </View>
                                                ))}
                                            </View>
                ...
                <View style={styles.modalOverlay}>
                    <View style={styles.modalSheet}>
                        <TouchableOpacity style={styles.closeBtn} onPress={closeModal}>
                            <Ionicons name="close-circle" size={32} color="#193648" />
                        </TouchableOpacity>
...
const styles = StyleSheet.create({
    ...
    loadingText: { marginTop: 15, fontSize: 16, color: "#193648", fontWeight: "600" },
    header: {
    ...
    headerIcon: {
        width: 60, height: 60, borderRadius: 30, backgroundColor: "#E8F6F3",
        justifyContent: "center", alignItems: "center", marginBottom: 10,
    },
    headerTitle: { fontSize: 22, fontWeight: "bold", color: "#193648", marginBottom: 5 },
    headerSubtitle: { fontSize: 14, color: "#7F8C8D", textAlign: "center" },
    skillsSection: {
        backgroundColor: "#fff", margin: 15, padding: 15, borderRadius: 12, elevation: 2,
    },
    sectionTitle: { fontSize: 16, fontWeight: "bold", color: "#193648", marginBottom: 10 },
    skillsContainer: { flexDirection: "row", flexWrap: "wrap" },
    skillBadge: {
        flexDirection: "row", alignItems: "center",
        paddingHorizontal: 10, paddingVertical: 6, borderRadius: 15,
        marginRight: 8, marginBottom: 8, backgroundColor: "#E8F6F3",
    },
    skillText: { fontSize: 12, color: "#193648", marginLeft: 4, fontWeight: "500" },
    infoBanner: {
        flexDirection: "row", backgroundColor: "#EBF5FB",
        marginHorizontal: 15, marginTop: 10, padding: 12, borderRadius: 10,
        alignItems: "center", borderLeftWidth: 4, borderLeftColor: "#3498DB",
    },
    infoText: { flex: 1, fontSize: 13, color: "#2C3E50", marginLeft: 10, lineHeight: 18 },
    recommendationHeader: { paddingHorizontal: 15, paddingVertical: 10 },
    recommendationCount: { fontSize: 16, fontWeight: "600", color: "#193648" },
    skillRow: { flexDirection: "row", flexWrap: "wrap" },
    skillSection: {
        paddingHorizontal: 15, paddingVertical: 10,
        borderTopWidth: 1, borderTopColor: "#EAECEF",
    },
    skillSectionTitle: { fontSize: 13, fontWeight: "600", color: "#7F8C8D", marginBottom: 8 },
    recommendationCard: {
        backgroundColor: "#fff", marginHorizontal: 15, marginBottom: 15,
        borderRadius: 15, elevation: 3, overflow: "hidden",
    },
    internshipImage: { width: "100%", height: 150, resizeMode: "cover" },
    cardHeader: { flexDirection: "row", padding: 15, backgroundColor: "#F8F9FA" },
    scoreCircle: {
    ...
    scoreText: { fontSize: 16, fontWeight: "bold" },
    cardHeaderText: { flex: 1, justifyContent: "center" },
    cardTitle: { fontSize: 16, fontWeight: "bold", color: "#193648", marginBottom: 4 },
    cardCompany: { fontSize: 14, color: "#7F8C8D" },
    cardActions: { flexDirection: "row", borderTopWidth: 1, borderTopColor: "#EAECEF" },
    applyButton: {
        flex: 1, flexDirection: "row", padding: 15,
        alignItems: "center", justifyContent: "center",
        backgroundColor: "#193648", gap: 6,
    },
    appliedButton: { backgroundColor: "#F0FDF4", borderWidth: 1, borderColor: "#A7F3D0" },
    applyButtonText: { fontSize: 14, color: "#fff", fontWeight: "600", marginLeft: 6 },
    emptyState: {
        alignItems: "center", justifyContent: "center",
        paddingVertical: 60, paddingHorizontal: 30,
    },
    emptyText: { fontSize: 18, fontWeight: "bold", color: "#193648", marginTop: 15 },
    emptySubtext: {
        fontSize: 14, color: "#7F8C8D", textAlign: "center", marginTop: 8, lineHeight: 20,
    },
    uploadButton: {
        marginTop: 20, backgroundColor: "#193648", paddingHorizontal: 25,
        paddingVertical: 12, borderRadius: 10, flexDirection: "row", alignItems: "center", gap: 8,
    },
    uploadButtonText: { color: "#fff", fontWeight: "600", fontSize: 14, marginLeft: 6 },
    notificationNote: {
        flexDirection: "row", alignItems: "center", marginTop: 20,
        backgroundColor: "#F8F9FA", paddingHorizontal: 15, paddingVertical: 10, borderRadius: 8,
    },
    notificationText: { fontSize: 12, color: "#7F8C8D", marginLeft: 8 },
    // Modal
    modalOverlay: {
    ...
    modalTitle: { fontSize: 18, fontWeight: "bold", color: "#193648", marginBottom: 4 },
    modalCompany: { fontSize: 14, color: "#7F8C8D", marginBottom: 16 },
    coverLabel: { fontSize: 15, fontWeight: "700", color: "#111827", marginBottom: 8 },
    coverLetterInput: {
    ...
    submitBtn: {
        backgroundColor: "#193648", paddingVertical: 15,
        borderRadius: 13, alignItems: "center", marginBottom: 10,
    },
    submitBtnText: { color: "#FFF", fontSize: 16, fontWeight: "bold" },
    cancelBtn: {
        backgroundColor: "#F3F4F6", paddingVertical: 13,
