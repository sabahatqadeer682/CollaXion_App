...
//                             // { label: "Approved", val: studentData?.approvedApplications || 0 },

// {
//   label: "Approved",
//   val: applications.filter(a => a.status === "approved").length || 0
// },
...
const StudentHomeScreen = () => {
    ...
    const fetchStudentStats = async () => {
        try {
            ...
            const serverImg = response.data?.profileImage;
            if (serverImg && (serverImg.startsWith("http") || serverImg.startsWith("data:"))) {
                ...
                // Server has no image — clear any stale value from a previous account
    ...
    const heroStats = [
        { label: "Skills", val: studentData?.extractedSkills?.length || 0, screen: "Profile Settings" },
        { label: "Applied", val: studentData?.totalApplications || 0, screen: "My Applications" },
        { label: "Approved", val: applications.filter((a: any) => a.status === "approved").length || 0, screen: "My Applications" },
        { label: "Alerts", val: notificationCount, screen: "Notifications" },
    ];
