...
const CustomDrawerContent = (props) => {
    ...
    const fetchStudentData = async () => {
        ...
        try {
            ...
            // Server is source of truth — clear stale context if it has nothing valid
            const serverImg = res.data.profileImage;
            if (serverImg && (serverImg.startsWith("http") || serverImg.startsWith("data:"))) {
                setProfileImage(serverImg);
                await AsyncStorage.setItem("studentProfileImage", serverImg);
            } else if (localImage && !localImage.startsWith("file://")) {
    ...
    const changeProfileImage = async () => {
        try {
            ...
            if (!result.canceled && result.assets && result.assets[0].uri) {
                ...
                const email = await AsyncStorage.getItem("studentEmail");
                if (email) {
                    // Convert to base64 so backend saves a proper data URI (not file://)
                    const base64 = await uriToBase64(selectedUri);
                    ...
                    // Save the base64 data URI locally so it persists across restarts
    ...
    return (
        ...
        >
            ...
            {/* ── White section (drawer items) ── */}
            <View style={styles.whiteSection}>
                <Animated.View style={{ opacity: fadeAnim }}>
                    <DrawerItemList {...props} />
                </Animated.View>

                <View style={styles.sepWhite} />
...
const StudentDashboardNavigator = () => {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            cleanupSocket = () => socket.off("newNotification", handleNewNotification);
        })();
    ...
    const sharedHeader = ({ navigation }) => ({
        ...
        headerStyle: { backgroundColor: "#193648", elevation: 0, shadowOpacity: 0 },
        headerTintColor: "#fff",
        // ── "CollaXion" text removed, only logo shown ──
        headerTitle: () => null,
        headerTitleAlign: "center" as const,
        headerLeft: () => (
            <View style={{ flexDirection: "row", alignItems: "center", marginLeft: 10 }}>
    ...
    return (
        ...
        >
            <Drawer.Screen name="Dashboard" component={StudentHomeScreen}
                options={(p) => ({ ...sharedHeader(p), ...ac("#193648"), drawerLabel: "Dashboard", drawerIcon: ({ color, size }) => <MaterialIcons name="dashboard" color={color} size={size} /> })} />

            <Drawer.Screen name="Internships" component={InternshipsScreen}
                options={(p) => ({ ...sharedHeader(p), ...ac("#193648"), drawerLabel: "Browse Internships", drawerIcon: ({ color, size }) => <MaterialCommunityIcons name="briefcase-search" color={color} size={size} /> })} />

            <Drawer.Screen name="My Applications" component={MyApplicationsScreen}
                options={(p) => ({ ...sharedHeader(p), ...ac("#193648"), drawerLabel: "My Applications", drawerIcon: ({ color, size }) => <MaterialIcons name="work-history" color={color} size={size} /> })} />

            <Drawer.Screen name="AI Recommendations" component={AIRecommendationsScreen}
                options={(p) => ({ ...sharedHeader(p), ...ac("#193648"), drawerLabel: "AI Recommendations", drawerIcon: ({ color, size }) => <MaterialIcons name="psychology" color={color} size={size} /> })} />

            <Drawer.Screen name="Nearby Industries" component={NearbyIndustriesScreen}
                options={(p) => ({ ...sharedHeader(p), ...ac("#193648"), drawerLabel: "Nearby Industries", drawerIcon: ({ color, size }) => <MaterialIcons name="location-on" color={color} size={size} /> })} />

            <Drawer.Screen name="Events" component={EventsScreen}
                options={(p) => ({ ...sharedHeader(p), ...ac("#193648"), drawerLabel: "Events", drawerIcon: ({ color, size }) => <MaterialCommunityIcons name="calendar-star" color={color} size={size} /> })} />

            <Drawer.Screen name="Feedback" component={FeedbackScreen}
                options={(p) => ({ ...sharedHeader(p), ...ac("#193648"), drawerLabel: "Feedback & Ratings", drawerIcon: ({ color, size }) => <MaterialCommunityIcons name="star-half-full" color={color} size={size} /> })} />

            <Drawer.Screen name="Profile Settings" component={ProfileScreen}
                options={(p) => ({ ...sharedHeader(p), ...ac("#193648"), drawerLabel: "My Profile", drawerIcon: ({ color, size }) => <MaterialIcons name="person" color={color} size={size} /> })} />

            <Drawer.Screen name="App Settings" component={SettingsScreen}
                options={(p) => ({ ...sharedHeader(p), ...ac("#193648"), drawerLabel: "Settings & Privacy", drawerIcon: ({ color, size }) => <MaterialIcons name="settings" color={color} size={size} /> })} />
{/* 
